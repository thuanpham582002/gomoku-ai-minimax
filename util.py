...
class Board:
    ...
    '''
    Hàm này tìm các nước đi có thể đi được từ vị trí (x, y) 
    và trả về danh sách các nước đi đó.
    '''
    def candidate(self):
        fives = list()
        fours = list()
        point_scores = list()
        '''
        Nếu đây là nước đi đầu tiên của trò chơi (self.step_count == 0), 
        trả về vị trí giữa bàn cờ: [(int(self.size[0] / 2), int(self.size[1] / 2))].
        '''
